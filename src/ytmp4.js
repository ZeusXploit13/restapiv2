...
const ytmp4 = {
  ...
  process: async (url) => {
    try {
      const info = await ytmp4.getInfo(url);
      if (!info) throw new Error("Gagal mendapatkan informasi video");

      const { title, thumbnail, formats } = info;
      if (!formats?.video?.mp4?.length)
        throw new Error("Format audio tidak tersedia");

      const token = formats.video.mp4[0].token;
      const convertData = await ytmp4.convert(token);
...
app.get('/download/ytmp4', async (req, res) => {
        try {
            ...
            const results = await ytmp4.process(url);
            res.status(200).json({
                status: true,
                result: {
                title: results.title, 
